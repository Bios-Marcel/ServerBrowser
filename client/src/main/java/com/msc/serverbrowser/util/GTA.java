...
public class GTA {
	...
	/**
	 * Writes the actual username (from registry) into the past usernames list
	 * and sets the new name
	 */
	...
	public static Optional<String> getInstalledVersion() {
		...
		if (path.isPresent()) {
			...
			/*
			 * Bad Practice, could potentionally cause an error if Kalcor
			 * decides to do a huge update someday :P
			 */
